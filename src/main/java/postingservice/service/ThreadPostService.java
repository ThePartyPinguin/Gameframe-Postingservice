...
public class ThreadPostService {
    ...
    public FullPostResponse createNewPost(long userId, CreatePostRequestDto dto){

        if(userId != dto.getUserId()){
            return new FullPostResponse(401, "invalid credentials");
        }
        ...
        {
            return new FullPostResponse(404, "User not found");
        }
        ...
        if(savedPost == null){
            return new FullPostResponse(502, "Something went wrong trying to save your post");
        }

        return new FullPostResponse(200, "Post saved", savedPost, response.getUserData(), new HashSet<>(), 0);
    }
